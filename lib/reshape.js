...
function init_matrix(graph){
    graph.nodes.forEach(function (node) {
            if (parseInt(node.id)>count)
                count = parseInt(node.id)
        });
    count = count+1;
    tmp = count;
    //初始化数组
...
function ori_matrix(graph){
    graph.links.forEach(function(edge){
        var i = parseInt(edge.source);
        var j = parseInt(edge.target);
        adjmatrix[i][j] = adjmatrix[j][i] = 1;
    });
    graph.nodes.forEach(function (node) {
        node.id = parseInt(node.id);
        node.weight = parseInt(node.value);
        promatrix[node.id][0] = node.id;
        promatrix[node.id][1] = node.name;
        promatrix[node.id][2] = node.symbolSize;
        promatrix[node.id][3] = node.attributes.modularity_class;
        promatrix[node.id][4] = degree(node.id,-1);
    });
...
function node_add(){
    ...
    //添加节点信息
    option.series[0].data.push({id: add_node_id, name: add_node_name,label:{normal:{show: true}}, weight: add_node_weight, symbolSize: 5*add_node_weight, draggable:true, value: add_node_weight, category: add_node_group});
...
function node_delete () {
    ...
    if(delete_node_id<count&&promatrix[delete_node_id][2]!=0){
        ...
        promatrix[delete_node_id][2]=null;
        promatrix[delete_node_id][3]=null;
        promatrix[delete_node_id][4]=null;
        update_table();
        window.addEventListener("resize", function () {
            mychart.resize(); 
            pocc_chart.resize();
